...
- (BOOL)isThroughTrainDate:(NSArray *)optionalDateArr {
    
    
    NSMutableArray *dateArrM = [NSMutableArray array];
    for (NSString *dateStr in optionalDateArr) {
        
        NSString *dateStr2 = @"";
        
        if ([dateStr isEqualToString:@"Sunday"]) {
            dateStr2 = @"星期天";
        }
        ...
        if ([dateStr isEqualToString:@"Tuesday"]) {
            dateStr2 = @"星期二";
        }
        
        if ([dateStr isEqualToString:@"Wednesday"]) {
            dateStr2 = @"星期三";
        }
        
        if ([dateStr isEqualToString:@"Thursday"]) {
            dateStr2 = @"星期四";
        }
        
        if ([dateStr isEqualToString:@"Friday"]) {
            dateStr2 = @"星期五";
        }
        
        if ([dateStr isEqualToString:@"Saturday"]) {
            dateStr2 = @"星期六";
        }
  
        [dateArrM addObject:dateStr2];
    }
    
    
    NSString *selfDateStr = [NSDate dayFromWeekday:self];
    
    for (NSString *dateStr in dateArrM) {
        
        if ([dateStr isEqualToString:selfDateStr]) {
            return YES;
        }
