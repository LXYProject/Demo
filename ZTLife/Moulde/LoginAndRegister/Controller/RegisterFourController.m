...
- (UITableViewCell *)sectionZeroWithTableView:(UITableView *)tableView
    ...
    if (_headImage) {
        ...
        [cell.headIcon sd_setImageWithURL:[NSURL URLWithString:[UserInfoManager sharedUserInfoManager].userInfoModel.userInfo.headImageUrl?[UserInfoManager sharedUserInfoManager].userInfoModel.userInfo.headImageUrl:@""] placeholderImage:[UIImage imageNamed:@"Oval 3 Copy"]];
    }
    return cell;
}
...
- (UITableViewCell *)sectionOneWithTableView:(UITableView *)tableView
    ...
    if (indexPath.row==0) {
        ...
        cell.detailTextLabel.text = [UserInfoManager sharedUserInfoManager].userInfoModel.userInfo.propertyUserName;
    }else if (indexPath.row==1){
        //cell.detailTextLabel.text = self.genderStr;
        if ([[UserInfoManager sharedUserInfoManager].userInfoModel.userInfo.gender integerValue]==0) {
...
- (UITableViewCell *)sectionThreeWithTableView:(UITableView *)tableView
    ...
    if (indexPath.row==0) {
        //cell.detailTextLabel.text = GetValueForKey(PhoneNumKey);
        //字符串的截取
        NSString *string = [[UserInfoManager sharedUserInfoManager].userInfoModel.userInfo.tel substringWithRange:NSMakeRange(3,4)];
        //字符串的替换
        cell.detailTextLabel.text = [[UserInfoManager sharedUserInfoManager].userInfoModel.userInfo.tel stringByReplacingOccurrencesOfString:string withString:@"****"];
    }
    cell.textLabel.textColor = TEXT_COLOR;
    cell.textLabel.text = _sectionThreeArr[indexPath.row];
    if (IS_IPHONE_4 || IS_IPHONE_5) {
