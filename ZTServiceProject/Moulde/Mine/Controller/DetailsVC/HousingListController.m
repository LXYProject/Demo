...
@interface HousingListController ()
...
- (void)searchHouse{
    @weakify(self);
    [MineHttpManager requestBuildingFloorId:self.buildingFloorId
...
//                                     }];
    // 绑定房屋
    @weakify(self);
    [MineHttpManager requestBindToCancelHouse:BindHouse machineId:[getUUID getUUID] machineName:[Tools deviceVersion] clientType:@"0" villageId:self.zoneId houseId:[self.dataSource[indexPath.row] houseId] success:^(id response) {
        @strongify(self);
        //操作失败的原因
        NSString *information = [response objectForKey:@"information"];
        //状态码
...
- (void)createAlertView{
    ...
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"绑定成功" preferredStyle:(UIAlertControllerStyleAlert)];
    // 创建按钮
