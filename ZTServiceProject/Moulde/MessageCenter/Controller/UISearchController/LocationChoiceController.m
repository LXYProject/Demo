...
@interface LocationChoiceController () <MAMapViewDelegate,MapPoiTableViewDelegate,AMapSearchDelegate,UISearchBarDelegate,SearchResultTableVCDelegate, AMapLocationManagerDelegate>
@end
...
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"发送" style:UIBarButtonItemStylePlain target:self action:@selector(sendLocation)];
    ...
    [self rightItemWithNormalName:@"" title:@"确定" titleColor:[UIColor whiteColor] selector:@selector(sendLocation) target:self];
...
//    [self.navigationController pushViewController:locationDetailVC animated:YES];
    ...
    [PushManager popViewControllerWithName:@"PostMessageController" animated:YES block:^(PostMessageController* viewController) {
        viewController.position = _tableView.selectedPoi.name;
    }];
    ...
    if (self.currentController==0) {
        [PushManager popViewControllerWithName:@"PostMessageController" animated:YES block:^(PostMessageController* viewController) {//发布帖子
            viewController.locationInfo = _tableView.selectedPoi.name;
        }];
