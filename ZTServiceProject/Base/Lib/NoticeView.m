...
@property (nonatomic, strong) NSArray *noticeGroup;
...
-(void)setLocalizationNoticeGroup:(NSArray *)localizationNoticeGroup {
    _localizationNoticeGroup = localizationNoticeGroup;
    ...
    if (_localizationNoticeGroup.count>0) {
        for (int i = 0; i < _localizationNoticeGroup.count ; i++) {
            
            UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
            btn.tag = BtnBaseTag + i;
            [btn setTitle:_localizationNoticeGroup[i] forState:UIControlStateNormal];
...
- (void)setIconImage:(NSString *)iconImage {
    ...
    self.imageView.image = [UIImage imageNamed:_iconImage];
