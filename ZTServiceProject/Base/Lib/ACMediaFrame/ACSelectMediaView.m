...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    return CGSizeMake(_mediaArray.count==0?self.width:self.width/4, self.width/4);
}
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    return _mediaArray.count==_maxCount?_mediaArray.count:_mediaArray.count+1;
}
...
/** 相册 */
- (void)openAlbum {
    TZImagePickerController *imagePickerVc = [[TZImagePickerController alloc] initWithMaxImagesCount:_maxCount - _mediaArray.count delegate:self];
    ///是否 在相册中显示拍照按钮
